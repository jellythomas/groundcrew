...
import { getNextTask, addTask, populateQueue, markTaskDone, listPending, listCompleted, cacheActiveTask, } from "./queue.js";
import { getFeedback, initFeedbackFile } from "./feedback.js";
import { reportStatus, incrementCompleted, endSession, getStatus, updateSession, } from "./session.js";
...
function sessionAge() { return sessionStartedAt ? Date.now() - sessionStartedAt : 0; }
function idleTime() { return lastTaskAt ? Date.now() - lastTaskAt : 0; }
function isOvertime() { return MAX_LIFETIME > 0 && sessionAge() >= MAX_LIFETIME; }
...
`.trim();
const server = new Server({ name: "groundcrew", version: "0.1.0" }, {
    capabilities: { tools: {} },
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "get_task",
            description: "Get the next task from the Groundcrew queue. BLOCKS silently until a task arrives or session times out (90 min idle). " +
        ...
        {
            name: "get_feedback",
            description: "Check for user feedback mid-task. Blocks briefly. Returns null if no feedback. " +
        ...
        {
            name: "mark_done",
            description: "Mark the current task as complete. IMPORTANT: After calling this, you MUST " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    output: {
                        type: "string",
                        description: "Full detailed output of the task — include your complete response, findings, " +
        ...
        {
            name: "report_status",
            description: "Report progress on the current task. Use for long-running tasks to keep " +
        ...
        {
            name: "populate_queue",
            description: "Add multiple tasks to the queue at once. Use when decomposing a large task " +
        ...
        {
            name: "list_queue",
            description: "List all pending tasks in the queue. Non-blocking. " +
        ...
        {
            name: "start",
            description: "MANDATORY: Call this tool IMMEDIATELY when the user's message contains the word 'groundcrew' " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case "get_feedback": {
            const timeout = args?.timeout_ms || FEEDBACK_TIMEOUT;
        ...
        case "mark_done": {
            ...
            if (!taskId || !summary) {
                return {
                    content: [
                        { type: "text", text: "Error: task_id and summary are required." },
                    ],
                    isError: true,
                };
            }
        ...
        case "report_status": {
            ...
            if (!taskId || !message) {
                return {
                    content: [
                        { type: "text", text: "Error: task_id and message are required." },
                    ],
                    isError: true,
                };
            }
